...
size_t sprav_heap_size = 32 * 1024;
K_HEAP_DEFINE(sprav_heap, 32 * 1024);
void *sprav_malloc(size_t size)
{
	...
	ptr = k_heap_alloc(&sprav_heap, size, K_FOREVER);
	memset(ptr, 0, size);
	return ptr;
}
				  ...
				  uint8_t *signature)
{
	...
	OQS_STATUS rc;
	size_t sig_len = 0;
	rc = OQS_SIG_dilithium_2_sign(signature, &sig_len, hash, 2, sprav_attestation_key);
	printk("liboqs_errno: %d\n", liboqs_errno);
	printk("Signature creation: %d\n", rc);
	printk("Signature length: %d\n", sig_len);

	rc = OQS_SIG_dilithium_2_verify(hash, 2, signature, sig_len, public_key);
